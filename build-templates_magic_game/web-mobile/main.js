window.boot = function () {
	var rootUrl =
		location.hostname === "localhost"
			? ""
			: "//j1.58cdn.com.cn/magic/magic-game/web-mobile/";
	var rootUrlSrc =
		location.hostname === "localhost"
			? ""
			: "//j1.58cdn.com.cn/magic/magic-game/web-mobile/src/";
	...
	var onStart = function () {
		...
		function hideTitleHandle() {
			WBAPP.invoke("toggle_title_panel", {
				contain_status_bar: true,
				cmd: "hide",
				keep_through: true,
			});
			WBAPP.invoke("set_status_bar", {
				mode: "light",
			});
		}
		hideTitleHandle();
		...
		function loadScene(launchScene) {
			cc.director.loadScene(launchScene, function (err) {
				if (!err) {
					...
					// 	clearInterval(window.fakeProgressTimer);
					// 	window.fakeProgressTimer = undefined;
					// 	window.fakeProgressPrecent = undefined;
					// }
	...
	if (jsList) {
		...
		jsList.push(rootUrl + bundledScript);
	} else {
		jsList = [rootUrl + bundledScript];
	}
	...
	cc.AssetLibrary.init({
		libraryPath: rootUrl + "res/import",
		rawAssetsBase: rootUrl + "res/raw-",
